...
- 인증은 `x-api-key: <key>` + `anthropic-version: 2023-06-01` 헤더를 사용합니다.
- upstream 경로는 `{base}/messages`입니다.
...
- 기본으로 23개를 동봉해뒀고, 다음 그룹으로 분류돼 있습니다:
  - **Anthropic 네이티브** (passthrough): Anthropic.
...
| POST   | `/v1/responses`         | OpenAI Responses → chat/completions 변환 (스트리밍 미지원)     |
